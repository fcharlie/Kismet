...
static inline void BinaryToHex(const unsigned char *buf, size_t len,
                               std::wstring &str) {
...
static inline void BinaryToHexUCase(const unsigned char *buf, size_t len,
                                    std::wstring &str) {
  char to_hex[] = "0123456789ABCDEF";
  for (uint32_t i = 0; i < len; i++) {
    unsigned int val = buf[i];
...
class MD5Hashsum : public KisasumBase {
public:
...
class SHA1Sum : public KisasumBase {
public:
  void Initialize(int width) {
    (void)width;
    rhash_sha1_init(&ctx);
...
class SHADC1Sum : public KisasumBase {
public:
  void Initialize(int width) {
    (void)width;
    SHA1DCInit(&ctx);
...
class SHA256Sum : public KisasumBase {
public:
...
class SHA512Sum : public KisasumBase {
public:
  void Initialize(int w) {
    width = w;
    if (width == 384)
...
class SHA3Sum : public KisasumBase {
public:
  void Initialize(int w) {
    width = w;
    switch (width) {
