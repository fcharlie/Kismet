...
public:
	...
	bool IsArgument(const wchar_t *candidate, const wchar_t *longname,const wchar_t *shortname) {
		return (wcscmp(candidate, longname) == 0 || wcscmp(candidate, shortname) == 0);
	}
	bool IsArgument(
	...
	int ProcessArgvImpl(KisasumFilelist &filelist) {
		...
		for (; index_ < Argc_; index_++) {
			...
			if (arg[0] == '-') {
				...
				if (IsArgument(arg, L"--algorithm", sizeof("--algorithm") - 1, &argvalue)
